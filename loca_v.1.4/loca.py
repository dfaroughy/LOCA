...
class open_loca(object):

    def __init__(self, file, cuts=True, lhco_header=False):
        self.f=open(file,'r')
        self.cuts=cuts
        self.parameters={}
        ...
        for l in self.f:
            ...
            if self.cuts==False:
               self.cuts={ 'photon':[0.0,1e+10,1e+10],'e':[0.0,1e+10,1e+10],'mu':[0.0,1e+10,1e+10],'tau':[0.0,1e+10,1e+10],
                           'jet':[0.0,1e+10,1e+10],'bjet':[0.0,1e+10,1e+10],'met':[0.0,1e+10,1e+10]}
            ...
            if len(line)>3:
                ...
                if obj.is_photon:
                    ...
                    if remove_endcaps(obj):  # remove endcaps      
                        if (obj.pt>self.cuts[obj.typ][0] and obj.pt<self.cuts[obj.typ][1] and abs(obj.eta)<self.cuts[obj.typ][2]):
